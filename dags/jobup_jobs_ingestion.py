...
from job_matcher.jobup import JOBUP_REGIONS, build_scheduled_window
...
@dag(
    dag_id="jobup_jobs_ingestion",
    description=(
        "Collect JobUp jobs published in each 15-minute interval, vectorize "
        "descriptions and save them in Postgres."
    ),
    schedule="*/15 * * * *",
    start_date=datetime(2024, 1, 1, tz="UTC"),
    catchup=False,
    max_active_runs=1,
    tags=["jobs", "jobup", "pgvector"],
)
def jobup_jobs_ingestion():
    @task
    def setup_run():
        ...
        window_start, window_end = build_scheduled_window(
            interval_end,
            active_settings.timezone,
        )

        run_id = os.environ.get("AIRFLOW_CTX_DAG_RUN_ID")
        run_key = f"jobup_jobs_ingestion__{run_id}" if run_id else None
        ...
        logger.info(
            "Initialized JobUp interval %s -> %s; metadata=%s",
            window_start,
            window_end,
            metadata_path,
        )
    ...
    def collect_jobs(context: dict[str, Any]):
        ...
        logger.info("JobUp search collection result: %s", result)
        return result
    ...
    def filter_known_jobs(search_result: dict[str, Any]):
        ...
        logger.info("JobUp known-job filtering result: %s", result)
        return result
    ...
    def collect_details(filtered_result: dict[str, Any]):
        ...
        logger.info("JobUp detail collection result: %s", result)
        return result
    ...
    def prepare_payloads(detail_result: dict[str, Any]):
        ...
        logger.info("JobUp preparation result: %s", result)
        return result
    ...
    def vectorize_paragraphs(prepared_result: dict[str, Any]):
        ...
        logger.info("JobUp vectorization result: %s", result)
        return result
    ...
    def persist_results(
        ...
        logger.info("JobUp persistence result: %s", result)
        return result
